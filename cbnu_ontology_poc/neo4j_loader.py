...
def run_tests(driver: Driver) -> None:
    """Execute validation queries and print results."""
    with driver.session() as session:
        print("\n=== Test 1: Available recommended books for a student's current term ===")
        student_term = session.run(
            """
            MATCH (s:Student)-[:ENROLLED_IN]->(c:Course)-[:HELD_IN_TERM]->(t:Term)
            RETURN s.id AS studentId, s.name AS studentName, t.id AS termId, t.name AS termName
            ORDER BY rand()
            LIMIT 1
            """
        ).single()
        if student_term:
            rows = session.run(
                """
                MATCH (s:Student {id: $studentId})-[:ENROLLED_IN]->(c:Course)-[:HELD_IN_TERM]->(t:Term {id: $termId})
                ...
                LIMIT 10
                """,
                studentId=student_term["studentId"],
                termId=student_term["termId"],
            ).data()
            if rows:
                for row in rows:
                    print(
                        f"{row['studentName']} | {row['term']} | {row['courseName']} "
                        f"-> {row['bookTitle']} ({row['callNumber']})"
                    )
            else:
                print("No available recommended books found for the sampled student.")
        else:
            print("No enrollment data found.")

        print("\n=== Test 2: Track/year-based noncurricular and scholarship bundle ===")
        track_record = session.run(
            """
            MATCH (mt:MajorTrack)
        ...
        if track_record:
            ...
            rows = session.run(
                """
                MATCH (track:MajorTrack {name: $trackName})
                MATCH (track)<-[:SUITABLE_FOR_MAJOR]-(program:NonCurricularProgram)
                OPTIONAL MATCH (program)<-[:REQUIRES_PROGRAM]-(sch:Scholarship)
                WHERE $year >= program.minYear AND $year <= program.maxYear
                OPTIONAL MATCH (sch)-[:REQUIRES_COURSE]->(req:Course)
                RETURN track.name AS trackName,
                       program.name AS programName,
                       program.competency AS competency,
                       sch.name AS scholarshipName,
                       sch.minGpa AS requiredGpa,
                       sch.minCredits AS requiredCredits,
                       collect(DISTINCT req.courseCode) AS requiredCourses
                ORDER BY programName
                LIMIT 10
                """,
                trackName=track_record["name"],
                year=target_year,
            ).data()
            if rows:
                for row in rows:
                    print(
                        f"Track {row['trackName']} | Program {row['programName']} ({row['competency']}) "
                        f"| Scholarship: {row['scholarshipName']} | GPA≥{row['requiredGpa']} "
                        f"Credits≥{row['requiredCredits']} | Courses {row['requiredCourses']}"
                    )
            else:
                print("No bundle found for sampled track/year.")
        else:
            print("No major tracks present.")

        print("\n=== Test 3: Graduation-readiness snapshot ===")
        grad_student = session.run(
            """
        ...
        if grad_student:
            record = session.run(
                ...
                RETURN s.name AS studentName,
                       s.creditsEarned AS recordedCredits,
                ...
                """,
                studentId=grad_student["id"],
            ).single()
            courses = [course["name"] for course in record["courses"] if course]
            earned = sum(course["credits"] for course in record["courses"] if course and course.get("credits"))
            programs = [p for p in record["programs"] if p]
            scholarships = [s for s in record["scholarships"] if s]
            remaining_credits = max(record["requiredCredits"] - earned, 0)
            required_programs = 3
            remaining_programs = max(required_programs - len(programs), 0)
            print(f"Graduating student: {record['studentName']}")
            print(
                f" Earned credits: {earned} / Required: {record['requiredCredits']} "
                f"(remaining {remaining_credits})"
            )
            print(f" Courses ({len(courses)}): {', '.join(courses[:8])}{'...' if len(courses) > 8 else ''}")
            print(f" Programs ({len(programs)}): {', '.join(programs)}")
            print(f" Scholarships ({len(scholarships)}): {', '.join(scholarships)}")
            if remaining_programs > 0 or remaining_credits > 0:
                print(
                    f" Outstanding requirements -> Extra programs needed: {remaining_programs}, "
                    f"credit gap: {remaining_credits}"
                )
            else:
                print(" All graduation requirements satisfied under simplified rule.")
        else:
            print("No graduating student found.")

        print("\n=== Test 4: Graph structure sanity checks ===")
        ...
        paths = session.run(
            ...
            MATCH (c)-[:HAS_RECOMMENDED_BOOK]->(b:Book)
            MATCH (c)-[:RELATED_TO_PROGRAM]->(p:NonCurricularProgram)
            MATCH (p)<-[:REQUIRES_PROGRAM]-(sch:Scholarship)
            RETURN s.name AS student, c.name AS course, b.name AS book,
                   p.name AS program, sch.name AS scholarship
            LIMIT 3
            """
        ).data()
        print("\nSample Student→Course→Book→Program→Scholarship chains:")
        if paths:
            for row in paths:
                print(
                    f" {row['student']} -> {row['course']} -> {row['book']} "
                    f"-> {row['program']} -> {row['scholarship']}"
                )
        else:
            print(" No full chain found (unexpected).")
